...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        #添加代码
        self.txt_folder = opt.get('dataroot_txt', None)  # 新增txt文件目录
    ...
    def __getitem__(self, index):
        ...
        # 如果是train模式，加载txt文件和facecount
        coordinates = None
        facecount = 0
        if self.opt['phase'] == 'train' and self.txt_folder:
            image_name = os.path.splitext(os.path.basename(gt_path))[0]

            # 提取 image_name 和 facecount
            txt_path = None
            for txt_file in os.listdir(self.txt_folder):
                if txt_file.startswith(image_name):
                    txt_path = os.path.join(self.txt_folder, txt_file)
                    facecount = int(os.path.splitext(txt_file)[0].split('_')[-1])
                    break

            # 加载坐标
            if txt_path:
                coordinates = np.loadtxt(txt_path)
