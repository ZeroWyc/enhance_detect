...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB,C2f_MLCA,C2f_CA):
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in {VanillaNet_6}:
            c2 = args[0]
            args = args[1:]

        elif m in {RetinexFormer}:
            args = [ch[f], *args]

        elif m in {MLCA,CoordAtt}:
            args = [ch[f], *args]
        elif m is AIFI:
            args = [ch[f], *args]
